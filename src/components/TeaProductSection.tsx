  ...
  return (
    <div id="section-tea" className="bg-white rounded-2xl border border-stone-200 p-5 sm:p-7 shadow-xs">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between pb-4 mb-5 border-b border-stone-100 gap-3">
        ...
        <div className="flex items-center gap-1.5 bg-stone-100 p-1 rounded-xl shrink-0 self-start sm:self-auto">
          <button
            type="button"
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-medium transition cursor-pointer flex items-center gap-1 whitespace-nowrap ${
              data.isCombination
      ...
      <div className="mb-5 p-3.5 rounded-xl bg-emerald-50/70 border border-emerald-200/60 flex items-center justify-between flex-wrap gap-2 text-xs">
        <div className="flex items-center gap-2 text-emerald-900 min-w-0 flex-wrap">
          <Scale className="w-4 h-4 text-emerald-700 shrink-0" />
          <span className="font-semibold whitespace-nowrap">{t('tea.itemSummary', '本礼盒包含')}：</span>
          <span className="font-mono bg-white px-2.5 py-1 rounded-md border border-emerald-300 text-emerald-950 font-medium break-all">
            {data.items.map(i => `${i.teaName || 'Tea'} × ${i.quantityInBox} × ${i.unitWeight}g`).join(' + ')}
          </span>
        </div>
        <div className="flex items-center gap-3 text-emerald-800 font-medium shrink-0 whitespace-nowrap">
          <span>{t('tea.cansCount', '礼盒内放置数量')}: <strong>{totalUnitsPerBox}</strong></span>
          <span>{t('tea.totalWeightInBox', '单套礼盒内茶叶总净含量：')} <strong>{totalWeightPerBox}g</strong></span>
        </div>
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-stone-700 flex items-center gap-1.5">
                <span className="w-5 h-5 rounded-full bg-emerald-100 text-emerald-800 flex items-center justify-center text-[11px] shrink-0">
                  {index + 1}
                </span>
              ...
              {data.items.length > 1 && (
                <button
                  ...
                  className="text-stone-500 hover:text-red-500 transition p-1 cursor-pointer"
                  title="Remove"
                >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-12 gap-3">
              ...
              <div className="xl:col-span-3 min-w-0">
                <label className="block text-[11px] font-semibold text-stone-700 mb-1 whitespace-nowrap">
                  {t('tea.category', '茶叶品类')} <span className="text-red-500">*</span>
                </label>
                <select
                  value={item.category}
                  onChange={(e) => handleUpdateItem(item.id, { category: e.target.value as TeaCategory })}
                  className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-stone-300 bg-white focus:ring-1 focus:ring-emerald-600 truncate"
                >
              ...
              <div className="xl:col-span-3 min-w-0">
                <label className="block text-[11px] font-semibold text-stone-700 mb-1 whitespace-nowrap">
                  {t('tea.name', '茶叶名称')} <span className="text-red-500">*</span>
                </label>
                <input
                  type="text"
                  value={item.teaName}
                  ...
                  className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-stone-300 bg-white focus:ring-1 focus:ring-emerald-600 font-medium"
                />
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input
                      ...
                      className="w-full px-2 py-1.5 text-xs rounded-lg border border-stone-300 bg-white pr-4"
                    />
                  </div>
                </div>
      ...
      <div className="mt-4 pt-3 border-t border-stone-100 flex items-center gap-2 flex-wrap">
        <span className="text-[11px] font-semibold text-stone-700 flex items-center gap-1 whitespace-nowrap">
        ...
        {POPULAR_TEAS.map((pt) => (
          <button
            ...
            className="text-[11px] px-2.5 py-1 rounded-md bg-stone-100 hover:bg-emerald-50 hover:text-emerald-800 text-stone-700 border border-stone-200 transition cursor-pointer whitespace-nowrap"
          >
            {pt.name}
          </button>
        ))}
