...
export const PackagingSection: React.FC<PackagingSectionProps> = ({
  ...
  const toggleCraft = (craftKey: string) => {
    const current = data.surfaceFinishes || [];
    const next = current.includes(craftKey)
      ? current.filter(c => c !== craftKey)
      : [...current, craftKey];
    onChange({ surfaceFinishes: next });
  };

  const selectedCraftsText = t('packaging.selectedCrafts', '已选 {n} 项工艺').replace('{n}', String(data.surfaceFinishes.length));

  return (
    <div id="section-pkg" className="bg-white rounded-2xl border border-stone-200 p-5 sm:p-7 shadow-xs">
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
          {BOX_CATEGORIES.map((cat) => {
            const isSelected = data.packagingType === cat.key;
            return (
              <button
                key={cat.key}
                type="button"
                onClick={() => onChange({ packagingType: cat.key })}
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1">
                    ...
                    <span className="text-[10px] px-1.5 py-0.5 rounded bg-stone-200 text-stone-700 font-medium whitespace-nowrap shrink-0">
                      {cat.badge}
                    </span>
                  </div>
            ...
            return (
              <button
                ...
                className={`p-3.5 rounded-xl border text-left transition cursor-pointer relative flex flex-col justify-between ${
                  isSelected
                    ? 'border-emerald-700 bg-emerald-50/80 shadow-xs ring-1 ring-emerald-700'
              ...
              >
                <div>
                  <div className="flex items-start justify-between gap-1.5 mb-1.5">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded font-semibold whitespace-nowrap shrink-0 ${model.tagColor}`}>
                      {model.tag}
                    </span>
                  </div>
                  <p className="text-[11px] text-stone-700 leading-relaxed mb-2.5 line-clamp-2">
                    {model.desc}
                  </p>
                </div>
                
                <div className="pt-2 border-t border-stone-100 flex items-center justify-between text-[11px] text-stone-700">
                  <span className="whitespace-nowrap">{t('packaging.dimLength', '推荐外寸')}:</span>
                  <span className="font-mono font-medium text-emerald-800 whitespace-nowrap">
      ...
      {/* 3. 尺寸与用料精细化 */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 rounded-xl bg-stone-50/50 border border-stone-200 mb-6">
        {/* Custom Dimensions */}
        <div>
          <label className="block text-xs font-semibold text-stone-700 mb-2">
          ...
          <div className="grid grid-cols-3 gap-2">
            <div>
              <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.dimLength', '长 (mm)')}</span>
            ...
            <div>
              <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.dimWidth', '宽 (mm)')}</span>
              <div className="relative">
                <input
                  type="number"
            ...
            <div>
              <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.dimHeight', '高 (mm)')}</span>
              <div className="relative">
                <input
                  type="number"
          ...
          <p className="text-[11px] text-stone-700 mt-1.5">
            {t('packaging.volumeCalc', '体积:')} {(((data.dimensions?.length || 300) * (data.dimensions?.width || 200) * (data.dimensions?.height || 80)) / 1000000).toFixed(2)} {t('packaging.volumeUnit', '立方分米 (影响运费与装箱数)')}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
            <div>
              <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.material', '礼盒核心用料 / 材质')}</span>
              <select
                value={data.material}
                onChange={(e) => onChange({ material: e.target.value })}
                className="w-full px-2.5 py-1.5 text-xs rounded-lg border border-stone-300 bg-white font-medium truncate"
              >
                {MATERIALS.map(m => (
            ...
            <div>
              <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.color', '盒面主色调 / 艺术风格')}</span>
              <input
                type="text"
                value={data.boxColor}
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            {COLOR_THEMES.map(c => (
              <button
                ...
                className="text-[10px] px-2 py-0.5 rounded-full border border-stone-200 bg-white hover:bg-stone-50 flex items-center gap-1 cursor-pointer whitespace-nowrap shrink-0"
              >
      ...
      <div className="mb-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[11px] text-emerald-700 font-medium whitespace-nowrap">{selectedCraftsText}</span>
        </div>
            ...
            return (
              ...
              >
                <div>
                  ...
                  <p className="text-[11px] text-stone-700 mb-1.5 leading-snug line-clamp-2">{sc.desc}</p>
                </div>
                <span className="text-[10px] font-mono text-amber-700 bg-amber-50 px-1.5 py-0.5 rounded w-fit whitespace-nowrap">
                  {sc.feeHint}
                </span>
              </button>
            );
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-5 gap-2">
          {INNER_TRAYS.map(tray => {
            ...
            return (
              ...
              >
                <div className="text-xs font-bold text-stone-900 mb-0.5 leading-tight">{tray.label}</div>
          ...
          {data.needHandbag ? (
            <div className="space-y-2 text-xs animate-in fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                  <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.handbagStyle', '手提袋工艺与提手')}</span>
                  <select
                    value={data.handbagHandleType}
                    onChange={(e) => onChange({ handbagHandleType: e.target.value })}
                    className="w-full px-2 py-1.5 text-xs rounded border border-stone-300 bg-white truncate"
                  >
                ...
                <div>
                  <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.handbagCraftLabel', '袋身工艺与纸张')}</span>
                  <input
                    type="text"
        ...
        <div className="p-4 rounded-xl border border-stone-200 bg-stone-50/50">
          ...
          {data.needOuterBox ? (
            <div className="space-y-2 text-xs animate-in fade-in">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <div>
                  <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.outerBoxPrint', '外箱版面印刷类型')}</span>
                  <select
                ...
                <div>
                  <span className="block text-[10px] text-stone-700 mb-0.5 whitespace-nowrap">{t('packaging.outerBoxSub', '装箱规数 (每箱装套数)')}</span>
                  <select
